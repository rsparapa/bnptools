...
namespace DPM {
  ...
  double mutau_F(double y, double mu, double tau) {
    // N(mu, 1./tau) 
    return R::dnorm(y, mu, pow(tau, -0.5), 0);
    //return sqrt(tau)*exp(-0.5*tau*pow(y-mu, 2.));
  }

  double mutau_G0tau(double a0, double b0, DPM::rng &eng) {
    return eng.rgamma(a0, b0);
  }

  double mutau_G0mu(double tau, double m0, double k0, DPM::rng &eng) {
    return eng.rnorm(m0, 1./sqrt(k0*tau));
  }

  //void mutau_P0(size_t row_c, Eigen::VectorXd &Y, SEXP _phi, 
  void mutau_P0(size_t row_c, SEXP _Y, SEXP _phi, 
...
*/
  ...
  RcppExport SEXP call_mutau(SEXP _Y, SEXP _phi, SEXP _C, SEXP _states, 
	...
	DPM::neal7(_Y, _phi, _C, _states, _prior, _hyper, eng,
		   &DPM::mutau_F, &DPM::mutau_G0tau,
		   &DPM::mutau_G0mu, &DPM::mutau_P0);
      else  
