...
//init
   ...
   Eigen::VectorXd mu(p);
   mu(p).setConstant(mub);
   Eigen::MatrixXd Sigma(Eigen::MatrixXd::Identity(p,p)/taub);
   bv=rnormXd(mu, Sigma);

   Eigen::Map<const Eigen::MatrixXd> xm(ix,p,n);
   //update Sigma
   Eigen::MatrixXd Tau((MatrixXd(p,p).setZero().selfadjointView<Lower>().rankUpdate(xm)));
   Tau = Tau*taub + Sigma.inverse();
   Sigma = Tau.inverse();
...
*/
  ...
  Eigen::Map<Eigen::VectorXd> zv(iz,n), bf(bm.f,n);
