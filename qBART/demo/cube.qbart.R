...
post <- abart(x.train=tdata[,c("age", "gender")], times=tdata$obstime, delta=tdata$event)
...
total.surv.mean <- apply(total.surv.train, 2, mean)
par(mfrow=c(1,2))
plot(c(0,postbc$times),c(1,total.surv.mean),type='s')
...
total.surv.mean <- apply(total.surv.train, 2, mean)
par(mfrow=c(1,2))
plot(c(0,postl$times),c(1,total.surv.mean),type='s')
