...
f = function(x, P) {
    j=seq(1, P, 5)
    sin(pi*apply(x[ , j], 1, mean)*apply(x[ , j+1], 1, mean)) +
        2*(apply(x[ , j+2], 1, mean)-0.5)^2 +
        apply(x[ , j+3], 1, mean) +
        0.5*apply(x[ , j+4], 1, mean) - 1.5
}
...
y.train=(rnorm(N, f(x.train, P), sigma)>0)*1

file.='dense-pbart.rds'
if(file.exists(file.)) {
    post=readRDS(file.)
} else {
    post <- list()
...
if(figures!='NONE')
    dev.copy2pdf(file=paste(figures, 'dense-pbart.pdf', sep='/'))
dev.copy2pdf(file='dense-pbart.pdf')
