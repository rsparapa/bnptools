...
post3 = ip.gbart(x.train, y.train, x.test, shards=10,
                 sparse=TRUE, mc.cores=B, seed=12) ## for same strata?
...
abline(v=100, h=0)
abline(h=1, col='blue')
legend('topright', c('N=10000', 'BART', 'Modified LISA',
                     'Informative prior', 'True'),
       lty=1, col=c(0:2, 8, 4))
plot(x, f(x.test), type='l', xlab='x3', ylab='f(x3)', col=4, lwd=2)
...
legend('topleft', col=c(4, 1, 2, 8), lty=1,
       legend=c('True', 'BART', 'Modified LISA', 'Informative prior'), lwd=2)
dev.off()

## xinfo=rbind(post$treedraws$cutpoints[[1]])
## L=length(post$treedraws$cutpoints)
## if(L>1) for(i in 2:L) xinfo=rbind(xinfo, post$treedraws$cutpoints[[i]]) 
## dimnames(xinfo)[[1]]=names(post$treedraws$cutpoints)
## str(xinfo)
