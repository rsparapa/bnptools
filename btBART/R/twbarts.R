...
x.train <- rbind(x.train1,x.train2)
...
if(is.na(sigmaf)) {
   tau=(max(y.train)-min(y.train))/(2*k*sqrt(2*ntree))
} else {
   tau = sigmaf/sqrt(2*ntree)
}
#--------------------------------------------------
#call
res = .Call("ctwbarts",
...
##res$nkeeptreedraws=nkeeptreedraws
    ...
    res$varcount2.mean <- apply(res$varcount1, 2, mean)
    res$varprob2.mean <- apply(res$varprob1, 2, mean)
    res$rm.const <- rm.const
attr(res, 'class') <- 'twbarts'
return(res)
}
