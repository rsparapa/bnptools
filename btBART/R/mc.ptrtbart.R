...
)
{
  ...
  for(i in 1:mc.cores) {
    parallel::mcparallel({psnice(value=nice);
      ptrtbart(x.train1=x.train1,trt.train=trt.train,y.train=y.train,
  ...
  if(mc.cores==1 | attr(post, 'class')!='ptrtbart') return(post)
  else {
