...
class rn
{
 public:
  ...
  virtual v1d log_dirichlet(std::vector<double>& alpha) = 0; 
  virtual ~rn() {}
};
...
//  virtual size_t discrete() {return (dis)(gen);}
  ...
  virtual v1d log_dirichlet(v1d& alpha){
    ...
    for(size_t j=0;j<k;j++) {
      double gamP1=this->gamma(alpha[j]+1,1.);
      double un=this->uniform();
      draw[j]=std::log(gamP1)+std::log(un)/alpha[j];
      sum_gam+=std::exp(draw[j]);
      if(draw[j]<mn) mn=draw[j];
    }
    lse=std::log(sum_gam);
    for(size_t j=0;j<k;j++) draw[j]=draw[j]-lse;
      //draw[j]=::exp(draw[j]);
    return draw;
  }
 private:
...
class arn: public rn
{
 ...
 arn(unsigned int n1, unsigned int n2) {::set_seed(n1, n2);}
  ...
  {::get_seed(n1, n2);}
  virtual v1d log_dirichlet(std::vector<double>& alpha){
    size_t k=alpha.size();
    double mn=pow(10,6);
    double sum_gam=0.;
    double lse;
    v1d draw(k);
    for(size_t j=0;j<k;j++) {
      double gamP1=::rgamma(alpha[j]+1,1.);
      double un=this->uniform();
      draw[j]=std::log(gamP1)+std::log(un)/alpha[j];
      sum_gam+=std::exp(draw[j]);
      if(draw[j]<mn) mn=draw[j];
...
*/
  ...
  virtual v1d log_dirichlet(std::vector<double>& alpha){
    ...
    v1d draw(k);
    for(size_t j=0;j<k;j++) {
      double gamP1=R::rgamma(alpha[j]+1,1.);
      double un=this->uniform();
      draw[j]=std::log(gamP1)+std::log(un)/alpha[j];
      sum_gam+=std::exp(draw[j]);
      if(draw[j]<mn) mn=draw[j];
